      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['borrow:history:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import {delBorrow} from "@/api/borrow/historyBorrow";
import {listHistoryBorrow} from "@/api/borrow/historyBorrow";
export default {
  name: "Borrow",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listHistoryBorrow(this.queryParams).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除历史借阅记录图书为"' + row.book.bookName + '"的数据项？').then(function() {
        console.log(row.book.bookName);
