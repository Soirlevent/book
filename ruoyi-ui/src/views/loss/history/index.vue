<template>
  <div class="app-container">
    ...
    <!--        </el-form-item>-->
    <!--        <el-form-item label="支付金额" prop="lossMoney">-->
    <!--          <el-input v-model="form.lossMoney" placeholder="请输入支付金额" />-->
    <!--        </el-form-item>-->
    ...
    <!--        </el-form-item>-->
    <!--        <el-form-item label="支付时间" prop="updataTime">-->
...
<script>
import { listOrder, getOrder, delOrder, addOrder, updateOrder } from "@/api/loss/order";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.lossId)
    ...
    handleUpdate(row) {
      this.reset();
      const lossId = row.lossId || this.ids
      getOrder(lossId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除图书遗失订单编号为"' + lossIds + '"的数据项？').then(function() {
        return delOrder(lossIds);
      }).then(() => {
        this.getList();
